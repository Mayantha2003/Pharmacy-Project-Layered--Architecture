...
import lk.ijse.pharamacymanagementlayerdsystem.view.tdm.CustomerTM;
...
public class CustomerController implements Initializable {
    ...
    @FXML
    private TableColumn<CustomerTM,String> cus_Id_Column;
    @FXML
    private TableColumn<CustomerTM,String> cus_Name_Column;
    @FXML
    private TableColumn <CustomerTM,String>cus_Address_Column;
    @FXML
    private TableColumn <CustomerTM,String>cus_Contact_Column;
    @FXML
    private TableColumn <CustomerTM,String>cus_Email_Column;
    @FXML
    private TableColumn<CustomerTM, Void> ActionCol;
    @FXML
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        table_Customer.getSelectionModel().selectedItemProperty().addListener((
                observable, oldValue, newValue) -> {
            if (newValue != null) {
                setDataToFields((CustomerTM) newValue);
            }
        });
        ActionColumn();
        loadCustomerTable();
    ...
    private void setDataToFields(CustomerTM customer) {
            ...
            return new TableCell<>() {
...

                {
                    ...
                    deleteButton.setOnAction(event -> {
                        CustomerTM customer = (CustomerTM) getTableView().getItems().get(getIndex());
    ...
    private void handleActionDelete(CustomerTM customer) {
        ...
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.YES) {
                try {
                    boolean isDeleted = customerBO.deleteCustomer(customer.getCustomerId());
                    if (isDeleted) {
    ...
    private void handleSaveCustomer() {
        try {
            ...
            if (name.isEmpty() || !name.matches(CUSTOMER_NAME_REGEX)) {
                ...
                CustomerDTO customerDTO = new CustomerDTO(name,address,contact,email);
    ...
    private void handleCustomerUpdate() {
        try {
            ...
            if (id.isEmpty() || !id.matches(CUSTOMER_ID_REGEX)) {
                ...
                CustomerDTO customerDTO = new CustomerDTO(Long.parseLong(id), name,address,contact, email);
