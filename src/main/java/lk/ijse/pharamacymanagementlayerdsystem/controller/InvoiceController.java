...
public class InvoiceController implements Initializable {
    ...
    @FXML
    private TableView<InvoiceTM> cartTable;
    @FXML
    private TableColumn<InvoiceTM, Integer> cartNoCol;
    @FXML
    private TableColumn<InvoiceTM, String> cartProductCol;
    @FXML
    private TableColumn<InvoiceTM, String> cartBatchCol;
    @FXML
    private TableColumn<InvoiceTM, String> cartExpiryCol;
    @FXML
    private TableColumn<InvoiceTM, Integer> cartQtyCol;
    @FXML
    private TableColumn<InvoiceTM, Double> cartUnitPriceCol;
    @FXML
    private TableColumn<InvoiceTM, Double> cartLineTotal;
    @FXML
    private TableColumn<InvoiceTM, Void> cartActionsCol;
    ...
    private ObservableList<InvoiceTM> cartList = FXCollections.observableArrayList();

    InvoiceBO invoiceBO = (InvoiceBO) BOFactory.getInstance().getBO(BOFactory.BOTypes.INVOICE);
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        cartNoCol.setCellFactory(column -> new TableCell<InvoiceTM, Integer>() {
            @Override
        ...
        cartList.addListener((ListChangeListener<InvoiceTM>) c -> updateSummary());
        ...
        productCombo.getEditor().textProperty().addListener((obs, oldValue, newValue) -> {
            ...
            try {
                List<String> suggestions = batchBO.getProductSuggestionsStartingWith(typed);
        ...
        productCombo.setOnAction(event -> {
            ...
            try {
                List<String> batches = batchBO.searchAvailableBatches(selectedProduct.trim());
    ...
    private void loadBatchesForProduct(String productName) {
        try {
            List<String> batches = batchBO.searchAvailableBatches(productName);
            batchCombo.getItems().clear();
    ...
    private void handleBatchSelect() {
        ...
        try {
            Batch batch = batchBO.getBatchByDisplay(selected);
            if (batch != null) {
    ...
    private void loadInvoiceNumber() {
        try {
            String nextNumber = invoiceBO.generateNextInvoiceNumber();
    ...
    private void handleAddToCart() {
        ...
        try {
            ...
            InvoiceTM detail = new InvoiceTM();
    ...
    private void setupCartTable() {
        ...
        cartActionsCol.setCellFactory(param -> new TableCell<InvoiceTM, Void>() {
            private final Button removeBtn = new Button("Remove");

            {
                removeBtn.setStyle("-fx-background-color: #e74c3c; -fx-text-fill: white;");
                removeBtn.setOnAction(event -> {
                    InvoiceTM detail = getTableView().getItems().get(getIndex());
    ...
    private void updateSummary() {
        ...
        int qty = cartList.stream().mapToInt(InvoiceTM::getQuantity).sum();
        double subtotal = cartList.stream().mapToDouble(InvoiceTM::getLineTotal).sum();
    ...
    private void handleSaveInvoice() {
        ...
        try {
            ...
            if (selectedCustomer != null && !selectedCustomer.trim().isEmpty() && !selectedCustomer.equals("Cash Customer")) {
                ...
                try {
                    long customerId = customerBO.getCustomerIdByName(selectedCustomer);
            ...
            long savedInvoiceId = invoiceBO.saveInvoiceWithTransaction(invoice);
    ...
    private boolean validateLine() {
        ...
        try {
            ...
            Batch selectedBatch = batchBO.getBatchByDisplay(selectedDisplayText);
    ...
    public void printBill(long invoiceId, BigDecimal discountValue) {
        try {
            ...
            InputStream inputStream = getClass().getResourceAsStream("/lk/ijse/pharamacymanagementlayerdsystem/assests/report/Invoice.jrxml");
            if (inputStream == null) {
